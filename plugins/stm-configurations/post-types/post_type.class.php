...
class STM_PostType {
	...
	public static function registerPostType( $postType, $title, $args ) {
		...
		$labels      = array(
			'name'               => __( $pluralTitle, STM_CONFIGURATIONS ),
			'singular_name'      => __( $title, STM_CONFIGURATIONS ),
			'add_new'            => __( 'Add New', STM_CONFIGURATIONS ),
			'add_new_item'       => __( 'Add New ' . $title, STM_CONFIGURATIONS ),
			'edit_item'          => __( 'Edit ' . $title, STM_CONFIGURATIONS ),
			'new_item'           => __( 'New ' . $title, STM_CONFIGURATIONS ),
			'all_items'          => __( 'All ' . $pluralTitle, STM_CONFIGURATIONS ),
			'view_item'          => __( 'View ' . $title, STM_CONFIGURATIONS ),
			'search_items'       => __( 'Search ' . $pluralTitle, STM_CONFIGURATIONS ),
			'not_found'          => __( 'No ' . $pluralTitle . ' found', STM_CONFIGURATIONS ),
			'not_found_in_trash' => __( 'No ' . $pluralTitle . '  found in Trash', STM_CONFIGURATIONS ),
			'parent_item_colon'  => '',
			'menu_name'          => __( $pluralTitle, STM_CONFIGURATIONS )
		);
	...
	public static function addMetaBox( $id, $title, $post_types, $callback = '', $context = '', $priority = '', $callback_args = '' ) {
		...
		foreach ( $post_types as $post_type ) {
			$title                                     = empty( $title ) ? __( 'Options', STM_CONFIGURATIONS ) : $title;
		...
		foreach ( $fields as $key => $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'hidden':
					...
					break;
				case 'file':
					$file = __("No file chosen", STM_CONFIGURATIONS);
					if ($meta) {
					...
					echo '
						<div class="stm_metabox_image stm_metabox_file">
							...
							<input class="stm_upload_image upload_button_'. $key .' button-primary" type="button" value="' . __( 'Choose PDF', STM_CONFIGURATIONS ). '" />
							<a href="#" class="stm_remove_image button">' . __( 'Remove PDF', STM_CONFIGURATIONS ). '</a>
						</div>
							...
							jQuery(function($) {
								$(".upload_button_'. $key .'").click(function(){
									...
									var custom_uploader = wp.media({
										title   : "' . __( 'Select file', STM_CONFIGURATIONS ) . '",
										library : { type : "application/pdf"},
										button  : {
											text: "' . __( 'Attach', STM_CONFIGURATIONS ) . '"
										},
										multiple: false
									}).on("select", function () {
								...
								$(".stm_remove_image").click(function(){
									...
									$(this).closest(".stm_metabox_image").find(".custom_preview_file").text("'.__("No file chosen", STM_CONFIGURATIONS).'");
									return false;
								});
							});
						</script>
				...
				case 'stm-multiple-checkbox':
					foreach($field['options'] as $id => $name) {
						if(!empty($meta) and !empty($meta[$id])) {
							$checked = "checked='checked'";
						} else {
							$checked = "";
						}
						echo '<div class="labeled">';
				...
				case 'images':
					...
					echo '</div>'.
						 '<div class="stm-add-media button button-primary">'.__( "Add", STM_CONFIGURATIONS ).'</div>'.
						 '<script>
						    jQuery(function($) {
				...
				case 'image':
					...
					echo '
						<div class="stm_metabox_image">
							...
							<input class="stm_upload_image upload_button_'. $key .' button-primary" type="button" value="' . __( 'Choose Image', STM_CONFIGURATIONS ). '" />
							<a href="#" class="stm_remove_image button">' . __( 'Remove Image', STM_CONFIGURATIONS ). '</a>
						</div>
						<script type="text/javascript">
							jQuery(function($) {
								$(".upload_button_'. $key .'").click(function(){
									var btnClicked = $(this);
									var custom_uploader = wp.media({
										title   : "' . __( 'Select image', STM_CONFIGURATIONS ) . '",
										button  : {
											text: "' . __( 'Attach', STM_CONFIGURATIONS ) . '"
										},
										multiple: true
								...
								$(".stm_remove_image").click(function(){
									$(this).closest(".stm_metabox_image").find(".custom_upload_image").val("");
									$(this).closest(".stm_metabox_image").find("img").attr("src", "");
									return false;
								});
							});
						</script>
